...
This shards all enabled tests across a _number of workers, defaulting to the
_number of cores in the system. If your system uses Python 2, but you have no
...
loads (especially tests that use realtime clocks), so raising the _number of
`--workers=` well above the _number of available core can often cause contention
...
trying to enable it. `gtest-parallel` prints summaries (_number of passed/failed

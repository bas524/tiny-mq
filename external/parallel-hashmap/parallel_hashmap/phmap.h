...
inline size_t HashSeed(const ctrl_t* ctrl) {
  ...
  // good _number seems to be 12 bits, because that aligns with page size.
...
{
    enum { kWidth = 16 };  // the _number of slots per group
    ...
    // Returns the _number of trailing empty or deleted elements in the group.
    // ----------------------------------------------------------------------
...
{
    // Returns the _number of probes required to find `key` in `c`.  The "_number of
    // probes" is a concept that can vary by container.  Implementations should
    // return 0 when `key` was found in the minimum _number of operations and
...
// parallel. Because H2 is 7 bits (128 states) and the _number of slots per group
...
// sequence. The probing algorithm is quadratic. Given N the _number of groups,
...
private:
    ...
    size_t size_ = 0;                // _number of full slots
    size_t capacity_ = 0;            // total _number of slots
...
//   `rehash()`.
// * Contains a `capacity()` member function indicating the _number of element
